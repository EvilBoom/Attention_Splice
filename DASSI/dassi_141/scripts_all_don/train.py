...
def train(train_queue, valid_queue, model,criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('all_don_eval train %03d %e %f %f', step, objs.avg, top1.avg, top5.avg)
...
def infer(valid_queue, model, criterion):
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        top5.update(prec5.item(), n)
        if step % args.report_freq == 0:
            logging.info('all_don_eval valid %03d %e %f %f', step, objs.avg, top1.avg, top5.avg)
    return top1.avg, objs.avg
...
if __name__ == '__main__':
    flag = True
    if flag:
        parser = argparse.ArgumentParser("ALL_DON_EVAL SPLICE SITE CLASSIFICATION")
        ...
        parser.add_argument('--arch', type=str, default='all_don', help='which architecture to use')
    ...
    # 随机数设定 -- 以上
    # ——————————————————————————————————————————————————————————————————————————————————————————————————————
    criterion = nn.CrossEntropyLoss()   # 交叉熵损失函数设置
    criterion = criterion.cuda()
    genotype = eval("genotypes.%s" % args.arch)  # 使用的结构genotypes.all_don
    ...
    # ——————————————————————————————————————————————————————————————————————————————————————————————————————
    ''' 
    作者 测试 common data这个函数能不能正常返回两个list中的相同元素
    a = [1, 2, 3, 4, 5]
    b = [5, 6, 7, 8, 9]
    ...
    print(common_data(a, b))
    '''
    file_name1 ="/media/appadmin/MY_BOOK/data/splicing/asp_data_subset/proc_all/all_don_seq.txt"
    file_name2 ="/media/appadmin/MY_BOOK/data/splicing/asp_data_subset/proc_all/all_don_label.txt"
    # 加载数据 -- 并不存在 -- 已经处理过了，直接用处理过的数据
    Seq = [line.rstrip('\n')[:-1] for line in open(file_name1)]  # dna sequence list
    Raw_lab = [line.rstrip('\n') for line in open(file_name2)]  # dna label list
    n = len(Raw_lab)  # 统计样本数量
    lab_dic = {
        "1": np.array([0, 1]),
        "0": np.array([1, 0])
    }   # 最终使用多标签分类？
    LABEL = seq2num(Raw_lab, lab_dic)  # 得到 label
    sub_length = 1
    a = Seq[1]  # a 是 seq 第二个样本
    elements = element(a)  # elements 是一个去重后的list，应该是 a c g t
    mapping_dic = combination(elements, sub_length)  # 得到 a c g t 对应的 one-hot 向量
    H = np.array(range(142)).reshape(142, 1)  # 得到 142,1 大小的矩阵
    d_1, d_2 = H.shape
    ...
    for species in ['hs', 'athaliana', 'celegans', 'dmelanogaster']:
        file_test_seq = f'E:/gitpull/DNA/data/splice2deep/test_{species}_don_seq.txt'
        file_test_label = f'E:/gitpull/DNA/data/splice2deep/test_{species}_don_label.txt'
        ...
        for i in range(test_n):
            test_DATA_[i, :, :] = plot_hb_dna(seq=test_Seq[i], h_curve=H, sub_length=sub_length,
                                              map_dic=test_mapping_dic)
        test_IMG = test_DATA_
        ...
        vars()[test_sp_queue] = torch.utils.data.DataLoader(test_data, batch_size=args.val_size,
                                                            sampler=test_sampler_sep)

    best_acc = 0
    result = {}
    train_loss_, valid_loss_, train_don_, valid_don_ = [], [], [], []
    for epoch in range(1, args.epochs+1):
        lr = scheduler.get_last_lr()[0]
        logging.info('epoch %d lr %e', epoch, lr)
        logging.info('*****************************************************************')
        logging.info('| ALL_DON ARCH= {}'.format(genotype))
        logging.info('*****************************************************************')
        model.drop_path_prob = args.drop_path_prob * epoch / args.epochs
        train_acc, train_obj = train(train_queue, valid_queue, model, criterion, optimizer)  # 训练
        logging.info('-' * 89)
        logging.info('all_don_eval train_acc %f', train_acc)
        logging.info('all_don_eval train_loss %f', train_obj)
        logging.info('-' * 89)
        train_loss_.append(train_obj)
        train_don_.append(train_acc)
        valid_acc, valid_obj = infer(valid_queue, model, criterion)
        logging.info('-' * 89)
        logging.info('all_don_eval valid_acc %f', valid_acc)
        logging.info('all_don_eval valid_loss %f', valid_obj)
        logging.info('-' * 89)
        ...
        if valid_acc > best_acc:
            best_acc = valid_acc
            logging.info('*' * 89)
            logging.info('WOWWW !!!..ALL_DON MODEL EVAL JUST GOT BETTER AND IM AT %f', best_acc)
            logging.info('*' * 89)
            is_best = True
            ...
            # test_athaliana_acc, test_athaliana_obj, y_athaliana_pred, y_athaliana_true, athaliana_auc_pred = testinfer(
            #     test_athaliana_queue, model, criterion)
            # test_celegans_acc, test_celegans_obj, y_celegans_pred, y_celegans_true, celegans_auc_pred = testinfer(
            #     test_celegans_queue, model, criterion)
            # test_dmelanogaster_acc, test_dmelanogaster_obj, y_dmelanogaster_pred, y_dmelanogaster_true, dmelanogaster_auc_pred = testinfer(
            #     test_dmelanogaster_queue, model, criterion)
            # 测试不同生物，wait
            logging.info('=' * 89)
            logging.info('| Intermediate test results at epoch : %d |', epoch + 1)
            logging.info('Intermediate all_don test_acc %f', test_all_acc)
            # logging.info('Intermediate hs_don test_acc %f', test_hs_acc)
            # logging.info('Intermediate athaliana_don test_acc %f', test_athaliana_acc)
            # logging.info('Intermediate celegans_don test_acc %f', test_celegans_acc)
            # logging.info('Intermediate dmelanogaster_don test_acc %f', test_dmelanogaster_acc)
            logging.info('=' * 89)
            logging.info('=' * 89)
            logging.info('Intermediate all_don test_auc %f', all_auc_pred * 100)
            # logging.info('Intermediate hs_don test_auc %f', hs_auc_pred * 100)
            # logging.info('Intermediate athaliana_don test_auc %f', athaliana_auc_pred * 100)
            # logging.info('Intermediate celegans_don test_auc %f', celegans_auc_pred * 100)
            # logging.info('Intermediate dmelanogaster_don test_auc %f', dmelanogaster_auc_pred * 100)
            logging.info('=' * 89)
    ...
    if use_save:
       filename = 'ALL_DON_EVAL_DARTS_classifier_' + datetime.now().strftime("%d-%h-%m-%s") + '.pkl'
    ...
    logging.info('all_don_eval test_acc %f', test_acc)
    logging.info('all_don_eval test_loss %f', test_obj)
    test_f1 = metrics.f1_score(torch.stack(y_true).tolist(), y_pred, average='macro')
    logging.info('all_don_eval F1 Score %f', test_f1)
    logging.info('all_don_eval Precision, Recall and F1-Score...')
    logging.info(metrics.classification_report(torch.stack(y_true).tolist(), y_pred))
    logging.info('all_don_eval Confusion Matrix...')
    cm = metrics.confusion_matrix(torch.stack(y_true).tolist(), y_pred)
    logging.info(cm)
    logging.info('all_don_eval roc_auc_score...')
