...
parser = argparse.ArgumentParser("ALL ACC DATA SPLICE SITE CLASSIFICATION")
...
parser.add_argument('--save', type=str, default='ALL ACC', help='experiment name')
...
file_name1 ="/raid/project/darts_splice/data_splice/proc_all/all_acc_seq.txt"
file_name2 ="/raid/project/darts_splice/data_splice/proc_all/all_acc_label.txt"
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr):
  ...
  for step, (input, target) in enumerate(train_queue):
      ...
      if step % args.report_freq == 0:
        logging.info('all_acc_search train %03d %e %f %f', step, objs.avg, top1.avg, top5.avg)
...
def infer(valid_queue, model, criterion):
  ...
  for step, (input, target) in enumerate(valid_queue):
    ...
    if step % args.report_freq == 0:
      logging.info('all_acc_search valid %03d %e %f %f', step, objs.avg, top1.avg, top5.avg)
...
def testinfer(valid_queue, model, criterion):
  ...
  return top1.avg, objs.avg,y_pred,y_true,auc_pred
...
for epoch in range(1,args.epochs+1):
  ...
  logging.info('*****************************************************************')
  logging.info('*****************************************************************')
  logging.info('| ALL_ACC_SEARCH-ARCH{}= {}'.format(epoch,genotype))
  logging.info('*****************************************************************')
  logging.info('*****************************************************************')
  logging.info('*****************************************************************')
  ...
  logging.info('-' * 89)
  logging.info('all_acc_search train_acc %f', train_acc)
  logging.info('all_acc_search train_loss %f', train_obj)
  logging.info('-' * 89)
  train_loss_.append(train_obj)
  train_acc_.append(train_acc)
  valid_acc, valid_obj = infer(valid_queue, model, criterion)
  logging.info('-' * 89)
  logging.info('all_acc_search valid_acc %f', valid_acc)
  logging.info('all_acc_search valid_loss %f', valid_obj)
  logging.info('-' * 89)
  ...
  if valid_acc > best_acc:
    ...
    best_acc = valid_acc
    logging.info('*' * 89)
    logging.info('WOWWW !!!..ALL_ACC_SEARCH MODEL JUST GOT BETTER AND IM AT %f', best_acc)
    logging.info('*' * 89)
    is_best = True
...
if use_save:
   filename = 'ALL_ACC_SEARCH_DARTS_classifier_' + datetime.now().strftime("%d-%h-%m-%s") + '.pkl'
...
logging.info('all_acc_search test_acc %f', test_acc)
logging.info('all_acc_search test_loss %f', test_obj)
test_f1 = metrics.f1_score(torch.stack(y_true).tolist(),y_pred, average='macro')
logging.info('all_acc_search F1 Score %f', test_f1)
logging.info('all_acc_search Precision, Recall and F1-Score...')
logging.info(metrics.classification_report(torch.stack(y_true).tolist(), y_pred))
logging.info('all_acc_search Confusion Matrix...')
